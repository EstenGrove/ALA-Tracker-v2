...
const parseQuarter = (qtrString, year) => {
	if (qtrString === "Q1") {
...
export const useDates = (base = new Date()) => {
	...
	const setQuarterFromString = quarterString => {
		...
		setGlobalDates({
			...
			quarter: {
				quarterStart: startOfQuarter(qtrDate) + ` ${year}`,
				quarterEnd: endOfQuarter(qtrDate) + ` ${year}`,
				name: quarterString
			},
			month: {
...
export { days, months, quarters, quarterRange, parseMonth, parseQuarter };
