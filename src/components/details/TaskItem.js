...
const getTaskDescription = task => {
  ...
  return replaceNullWithMsg(task.Description, "No description"); // this should be TaskDescription, Description is often empty
};
    ...
    >
      ...
      <section className={styles.TaskItem_inner}>
        ...
        <header className={styles.TaskItem_inner_category}>
          ...
          >
            <use
              xlinkHref={`${sprite}#icon-${adlIcons[task?.ADLCategory ?? getCategoryNameFromID(task?.AssessmentCategoryId)].icon}`}
            ></use>
          </svg>
